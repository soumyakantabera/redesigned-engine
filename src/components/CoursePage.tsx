...
export function CoursePage({ data }: { data: CourseData }) {
  ...
  return (
    <Layout waMessage={waPrimary} footerImage={data.footerImage}>
      ...
      <FaqSection items={data.faqs ? [...data.faqs, ...GENERAL_FAQS] : GENERAL_FAQS} footerImage={data.footerImage} />
    </Layout>
  );
}
...
export function courseSeo(d: CourseData) {
  ...
  const allFaqs = d.faqs ? [...d.faqs, ...GENERAL_FAQS] : GENERAL_FAQS;
  if (allFaqs.length) {
    ld.push({
      "@context": "https://schema.org",
      "@type": "FAQPage",
      mainEntity: allFaqs.map((f, i) => ({
        "@type": "Question",
        name: f.q,
        acceptedAnswer: { "@type": "Answer", text: f.a },
        // Ensure unique @id to avoid Google Rich Results duplicate warnings
        "@id": `${url}#faq-${i}`,
      })),
    });
  }
  ...
  return {
    meta: [
      ...
      { name: "twitter:title", content: `${d.title} – Learn With Smile` },
